...
async function seedProductionDemo() {
  ...
  // 12. Seed Originality Reports and Synthetic Certificates for Completed Demo Projects
  await adminDb.collection("originalityReports").doc("cg_orig_7").set({
    id: "cg_orig_7",
    ...
    passed: true,
    flags: [],
    analysisMetadata: "Automated institutional evaluation verified unique sensor telemetry processing and route optimization algorithm.",
    createdAt: new Date(now - 1000 * 60 * 60 * 24 * 5).toISOString()
  }, { merge: true });

  await adminDb.collection("originalityReports").doc("cg_orig_8").set({
    id: "cg_orig_8",
    ...
    passed: true,
    flags: [],
    analysisMetadata: "Automated institutional evaluation verified unique multi-dimensional matching taxonomy.",
  ...
  const cert1: Certificate = {
    ...
    issuerId: userUIDs["faculty.demo@synergybridge.local"] || "faculty_demo_uid",
    ...
    eligibilitySnapshot: {
      ...
      originalityScore: 96,
      eligibilityCheckedAt: new Date(now - 1000 * 60 * 60 * 24 * 5).toISOString(),
    },
    createdAt: new Date(now - 1000 * 60 * 60 * 24 * 5).toISOString(),
  ...
  const cert2: Certificate = {
    ...
    course: "B.Tech",
    academicCredits: 4,
    issuedAt: new Date(now - 1000 * 60 * 60 * 24 * 2).toISOString(),
    status: CertificateStatus.ISSUED,
    ...
    eligibilitySnapshot: {
      ...
      completedMilestones: true,
      originalityScore: 92,
      eligibilityCheckedAt: new Date(now - 1000 * 60 * 60 * 24 * 2).toISOString(),
    },
    createdAt: new Date(now - 1000 * 60 * 60 * 24 * 2).toISOString(),
  ...
  await adminDb.collection("certificates").doc(cert2.id).set(cert2, { merge: true });

  console.log("✅ Seeded CropGuard AI tasks, milestones, messages, files, activities, funding & originality");
  console.log("✅ Seeded Synthetic Demo Certificates & Originality Reports for completed projects WasteWise and SkillMatch");
