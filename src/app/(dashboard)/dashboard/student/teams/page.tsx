  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white mb-2">My Teams</h1>
          <p className="text-slate-400">Manage your teams and invitations.</p>
        </div>
            ...
            {invitations.map(inv => (
              <Card key={inv.id}>
                <CardContent className="p-4 flex justify-between items-center">
                  <div>
                    <h3 className="font-medium text-white">Team Invitation</h3>
                    <p className="text-xs text-slate-400 mt-1">From user {inv.inviterId.slice(0,6)}</p>
                  </div>
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" className="text-emerald-400 border-emerald-400/20 hover:bg-emerald-950" onClick={() => handleInvitation(inv, true)}>
                      <Check className="w-4 h-4" />
                    </Button>
                    <Button size="sm" variant="outline" className="text-red-400 border-red-400/20 hover:bg-red-950" onClick={() => handleInvitation(inv, false)}>
                      <X className="w-4 h-4" />
                    </Button>
                  </div>
                </CardContent>
      ...
      <section className="space-y-4">
        ...
        {teams.length === 0 ? (
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-8 text-center">
            ...
            <p className="text-slate-400">You have not joined any teams yet.</p>
          </div>
        ) : (
              ...
              <Card key={team.id}>
                ...
                <CardContent>
                  <div className="text-sm text-slate-400 space-y-2 mb-6">
                    ...
                    <p>Status: <span className="text-slate-300">{team.status}</span></p>
                  </div>
