    ...
    const fetchProblem = async () => {
      try {
        ...
        if (snapshot.exists()) {
          ...
          // Only show if published, or if user is admin/poster
          if (
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-6 border-y border-slate-800">
            <div className="flex items-center gap-4">
              <div className="h-12 w-12 rounded-full bg-slate-800 flex items-center justify-center text-xl font-bold text-white border border-slate-700">
              ...
              <div>
                <p className="text-sm font-medium text-white flex items-center gap-2">
                  ...
                  {isVerified && (
                    <span className="flex items-center text-green-500 bg-green-500/10 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider" title="Verified Challenge">
                      <BadgeCheck className="w-3 h-3 mr-1" />
                      Verified
                    </span>
                  )}
                </p>
                <p className="text-xs text-slate-400 capitalize">Posted by {problem.posterRole.toLowerCase()}</p>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <Button 
                ...
                className={isBookmarked ? "text-blue-400 border-blue-400/50 hover:bg-blue-950" : ""}
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <div className="md:col-span-2 space-y-12">
            ...
            {problem.successCriteria && problem.successCriteria.length > 0 && (
              <section>
                <h2 className="text-xl font-bold text-white mb-4">Success Criteria</h2>
            ...
            {problem.constraints && problem.constraints.length > 0 && (
              <section>
                <h2 className="text-xl font-bold text-white mb-4">Constraints & Limitations</h2>
                <div className="grid gap-4">
                  {problem.constraints.map((constraint, idx) => (
                    <div key={idx} className="bg-slate-900/50 border border-slate-800 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-sm font-bold text-slate-200 capitalize">{constraint.type.replace('_', ' ')}</span>
                        <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded ${
                          constraint.severity === 'HIGH' ? 'bg-red-500/20 text-red-400' :
                          constraint.severity === 'MEDIUM' ? 'bg-yellow-500/20 text-yellow-400' :
                          'bg-blue-500/20 text-blue-400'
                        }`}>
                          {constraint.severity}
                        </span>
                      </div>
                      <p className="text-sm text-slate-400">{constraint.description}</p>
                    </div>
                  ))}
                </div>
              </section>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-6">
              <h3 className="font-bold text-white mb-4">Project Overview</h3>
              
              <div className="space-y-4">
                <div>
                  <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Difficulty</h4>
                  <p className="text-sm text-slate-200 font-medium capitalize">{problem.difficulty.toLowerCase()}</p>
                </div>
                
                <div>
                  <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Team Preference</h4>
                  <div className="flex items-center text-sm text-slate-200 font-medium capitalize">
                ...
                {problem.geographicScope && (
                  <div>
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Geographic Scope</h4>
                    <div className="flex items-center text-sm text-slate-200 font-medium capitalize">
                ...
                {problem.funding?.fundingEnabled && (
                  <div>
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Funding</h4>
              ...
              <div className="space-y-3">
                {problem.skills?.map((skill, idx) => (
                  <div key={idx} className="flex justify-between items-center text-sm">
                    ...
                    <span className="text-xs text-slate-500 capitalize">{skill.minimumLevel.toLowerCase()}</span>
                  </div>
                ))}
            ...
            {problem.sdgs && problem.sdgs.length > 0 && (
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                <h3 className="font-bold text-white mb-4">Impact Goals</h3>
                <div className="space-y-3">
                  {problem.sdgs.map(sdgId => {
                    ...
                    return (
                      <div key={sdg.id} className="flex items-center gap-3">
                        ...
                        <span className="text-sm text-slate-300 font-medium leading-tight">{sdg.name}</span>
                      </div>
                    );
                  })}
                </div>
