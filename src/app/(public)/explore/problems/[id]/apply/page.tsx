...
export default function ApplyPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        // Default apply type based on preference
        if (pData.teamPreference !== TeamPreference.INDIVIDUAL) {
          setApplyType("TEAM");
        }

        // 2. Fetch Profile & Check Completion
        ...
        // 5. Fetch eligible teams (where user is leader)
        const tQ = query(
          collection(db, "teams"),
  ...
  if (loading) return <div className="text-slate-400 p-8">Loading application...</div>;
  ...
  return (
    <div className="min-h-screen bg-slate-950 pt-24 pb-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 space-y-8">
        ...
        <div>
          <Link href={`/explore/problems/${id}`} className="flex items-center text-sm font-medium text-slate-400 hover:text-white transition-colors mb-6">
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back to Problem
          </Link>
          <h1 className="text-3xl font-bold text-white mb-2">Apply: {problem.title}</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {applyType === "TEAM" && (
                <div className="space-y-2">
                  ...
                  {eligibleTeams.length > 0 ? (
                    ...
                    <div className="text-amber-400 text-sm p-3 bg-amber-900/20 border border-amber-800/50 rounded">
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full bg-slate-900 border border-slate-800 rounded-md p-3 text-white h-48"
                  required
                  minLength={100}
                  maxLength={3000}
                />
                <p className="text-xs text-slate-500 flex justify-end">{proposal.length} / 3000</p>
              </div>

              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-300">Why are you a good fit?</label>
                <textarea
                  value={motivation}
                  onChange={(e) => setMotivation(e.target.value)}
                  placeholder="Explain why you or your team are uniquely suited... (min 50 chars)"
                  className="w-full bg-slate-900 border border-slate-800 rounded-md p-3 text-white h-24"
                  required
                  minLength={50}
                  maxLength={1500}
                />
                <p className="text-xs text-slate-500 flex justify-end">{motivation.length} / 1500</p>
              </div>
