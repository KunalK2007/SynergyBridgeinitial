...
if (process.env.NODE_ENV === 'production' && !envProjectId) {
  throw new Error("❌ CRITICAL: Firebase Project ID is missing in production. Refusing to silently connect to the development project.");
}

const TARGET_PROJECT_ID = envProjectId || 'synergybridgee-dev';
