...
const envSchema = z.object({
  ...
  // Operation Mode (NORMAL or RECOVERY)
  SYNERGYBRIDGE_OPERATION_MODE: z.enum(["NORMAL", "RECOVERY"]).default("NORMAL"),
});
...
if (env.NODE_ENV === "production") {
  if (!env.FIREBASE_SERVICE_ACCOUNT_KEY) {
    throw new Error("❌ CRITICAL: FIREBASE_SERVICE_ACCOUNT_KEY is missing in production. Refusing to boot.");
  }
