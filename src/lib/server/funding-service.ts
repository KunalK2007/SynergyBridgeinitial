...
export class FundingService {
  private checkRecoveryMode() {
    if (serverEnv.SYNERGYBRIDGE_OPERATION_MODE === "RECOVERY") {
      throw new Error("SynergyBridge is currently in Recovery Mode. Funding operations are temporarily paused.");
    }
  }
  ...
  async reviewFunding(
    ...
    return await adminDb.runTransaction(async (t: FirebaseFirestore.Transaction) => {
      ...
      const userSnap = await t.get(adminDb.collection("users").doc(reviewerId));
      if (!userSnap.exists) throw new Error("Reviewer not found.");
      const user = userSnap.data() as User;
      if (!user.isInstitutionVerified && user.role !== UserRole.ADMIN) {
        throw new Error("Unverified users cannot review funding requests.");
      }
  ...
  async disburseMilestone(
    ...
    return await adminDb.runTransaction(async (t: FirebaseFirestore.Transaction) => {
      ...
      const isGrowthOrInnovation = grant.tier === "GROWTH" || grant.tier === "INNOVATION";
      const approvals = milestone.approvals || { aiOriginalityPassed: false };
      
      if (!approvals.aiOriginalityPassed) {
        throw new Error("Cannot disburse: AI Originality approval missing.");
      }
      if (!approvals.mentorApprovedBy) {
        throw new Error("Cannot disburse: Mentor approval missing.");
      }
      if (isGrowthOrInnovation && !approvals.sponsorApprovedBy) {
        throw new Error(`Cannot disburse: Sponsor approval required for ${grant.tier} tier.`);
      }
  ...
  /**
   * Provide a signature for a milestone.
   */
  async signMilestone(
    grantId: string,
    milestoneId: string,
    signerId: string,
    signerName: string,
    signatureType: "AI" | "MENTOR" | "SPONSOR"
  ): Promise<FundingGrant> {
    this.checkRecoveryMode();
    return await adminDb.runTransaction(async (t: FirebaseFirestore.Transaction) => {
      const grantRef = adminDb.collection("fundingGrants").doc(grantId);
      ...
      const milestone = grant.milestones[milestoneIndex];
      if (!milestone.approvals) {
        milestone.approvals = { aiOriginalityPassed: false };
      }

      if (signatureType === "MENTOR" || signatureType === "SPONSOR") {
        const userSnap = await t.get(adminDb.collection("users").doc(signerId));
        if (!userSnap.exists) {
          throw new Error(`Signer ${signerId} not found.`);
        }
        const user = userSnap.data() as User;
        if (!user.isInstitutionVerified) {
          throw new Error(`Signer ${signerId} is not institutionally verified.`);
        }
        
        if (signatureType === "MENTOR") {
          if (user.role !== UserRole.MENTOR && user.role !== UserRole.FACULTY) {
            throw new Error(`Signer ${signerId} is not a valid mentor or faculty.`);
          }
        }
        
        if (signatureType === "SPONSOR") {
          const validSponsorRoles = [UserRole.INDUSTRY, UserRole.GOVERNMENT, UserRole.INCUBATION];
          if (!validSponsorRoles.includes(user.role)) {
             throw new Error(`Signer ${signerId} is not a valid sponsor.`);
          }
        }
      }

      if (signatureType === "AI") milestone.approvals.aiOriginalityPassed = true;
      if (signatureType === "MENTOR") milestone.approvals.mentorApprovedBy = signerId;
      if (signatureType === "SPONSOR") milestone.approvals.sponsorApprovedBy = signerId;
      ...
      t.set(activityRef, {
        ...
        action: ActivityType.FUNDING_SIGNATURE_ADDED,
        entityType: "FUNDING_SIGNATURE",
        entityId: grantId,
        metadata: { milestoneId, signatureType },
        createdAt: Date.now(),
      });
