...
export function RoleDashboard({
  ...
  const getFeedIcon = (type?: string) => {
    switch (type) {
      ...
      case "APPLICATION":
        return <Target className="w-5 h-5 text-[#9C7A4C]" />;
      case "PROJECT":
      default:
        return <Lightbulb className="w-5 h-5 text-[#9C7A4C]" />;
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-[#1C1C1E] mb-2">Welcome, {roleName}</h1>
        <p className="text-[#5B5F73]">{description}</p>
      </div>

      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card className="border-[#9C7A4C]/20 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-[#5B5F73]">Active Projects</CardTitle>
            <Lightbulb className="h-4 w-4 text-[#9C7A4C]" />
          </CardHeader>
          <CardContent>
            {stats?.loading ? (
        ...
        <Card className="border-[#9C7A4C]/20 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-[#5B5F73]">Matches</CardTitle>
            <Users className="h-4 w-4 text-[#9C7A4C]" />
          </CardHeader>
          <CardContent>
            {stats?.loading ? (
              <Skeleton className="h-8 w-16 mb-1" />
        ...
        <Card className="border-[#9C7A4C]/20 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-[#5B5F73]">Impact Score</CardTitle>
            <Target className="h-4 w-4 text-[#9C7A4C]" />
          </CardHeader>
          <CardContent>
            {stats?.loading ? (
      ...
      <div className="mt-8">
        <h2 className="text-xl font-semibold text-[#1C1C1E] mb-4">Your Feed</h2>
        {feedLoading ? (
              ...
              >
                <CardContent className="p-4 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3.5">
                    <div className="p-2.5 rounded-lg bg-[#F6F5F2] border border-[#5B5F73]/15 flex items-center justify-center">
                      {getFeedIcon(item.type)}
                    </div>
                    <div>
                      <h4 className="font-semibold text-sm text-[#1C1C1E]">{item.title}</h4>
                      <p className="text-xs text-[#5B5F73] mt-0.5">{item.description}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    {item.link && (
                      <Link
                        href={item.link}
                        className="inline-flex items-center gap-1 text-xs font-medium text-[#9C7A4C] hover:text-[#7A6039] p-1.5 hover:bg-[#9C7A4C]/10 rounded-md transition-colors"
                      >
